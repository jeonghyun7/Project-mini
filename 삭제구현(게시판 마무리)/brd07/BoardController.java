	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try {
					...
					if (action == null) {
							...
							PrintWriter pw = response.getWriter();
							pw.print("<script>" + " alert('새글을 삭제했습니다.');" + " location.href='" + 
									request.getContextPath() + "/board/listArticles.do';" + "</script>");
							return;
							...
							articlesList = boardService.listArticles(); //추가들어가야함
							request.setAttribute("articlesList", articlesList);	//추가들어가야함
							nextPage = "/board06/listArticles.jsp";
					}
		...
		try
		{
			...
			for (int i = 0; i < items.size(); i++)
			{
				...
				} else
				{
					...
					//위 코드 한줄은 익스플러로에서 업로드 파일의 경로제거후 map에 파일명 저장
